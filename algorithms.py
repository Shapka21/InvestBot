...
def min_volantily(stocks, money):
    ...
    Sigma = risk_models.sample_cov(data['Close'])

    ef1 = EfficientFrontier(mu, Sigma, weight_bounds=(0, 1))
    ef1.min_volatility()
    minvol_pwt = ef1.clean_weights()

    latest_prices = get_latest_prices(data['Close']) * 10
...
def info_stock(stock):
    ...
    if len(data) != 0:
        mu = expected_returns.mean_historical_return(data['Close'])
        Sigma = risk_models.sample_cov(data['Close'])
        last_prise = data['Close'][-1]
        return mu['Close'] * 100, Sigma['Close']['Close'], last_prise, True
    ...
    if len(data) != 0:
        ...
        Sigma = risk_models.sample_cov(data['Close'])
        last_prise = data['Close'][-1]
        return mu['Close'] * 100, Sigma['Adj Close']['Adj Close'], last_prise, True
    else:
        return 0, 0, 0, False
